...
const API_BASE = `${window.location.protocol}//${window.location.hostname}:5000/api/auth`;
const API_USER = `${window.location.protocol}//${window.location.hostname}:5000/users`;
...
// Update user info (fullname, dob, address, avatar)
...
export const changeUserPassword = async (userId, passwordData) => {
    try {
        return await axios.put(`${API_BASE}/user/${userId}/change-password`, passwordData);
    } catch (err) {
