...
export default {
  ...
  computed: {
    ...
    cartItemCount() {
      // Assuming cart items are stored in localStorage as a JSON array
...
/* Cart Icon */
.nav-links li i {
  font-size: 25px; /* Increase the size of the icon */
  margin-right: 10px; /* Add space between the icon and text */
  color: #fff; /* Set the default icon color */
  transition: color 0.3s ease, transform 0.3s ease; /* Add smooth transition */
  position: relative; /* Position the badge */
}

/* Cart Icon Hover Effects */
.nav-links li i:hover {
  color: #00b33c; /* Change the icon color on hover */
  transform: scale(1.2); /* Slightly increase the icon size on hover */
}

/* Cart Icon Badge */
.nav-links li i::after {
  content: attr(data-count); /* The count from the data-count attribute */
  position: absolute;
  top: -5px;
  right: -10px;
  background-color: #ff4d4d; /* Red color for the badge */
  color: white;
  border-radius: 50%;
  padding: 2px 7px;
  font-size: 12px;
